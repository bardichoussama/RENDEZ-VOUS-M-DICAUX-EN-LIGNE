                    ...
                    <div class="">
                        <div class="relative overflow-x-auto sm:rounded-sm">
                            ...
                            <table class="w-full text-sm text-left text-gray-500 rtl:text-right dark:text-gray-400">
                                ...
                                <tbody>
                                    @forelse ($todayConsultationsList as $consultation)
                                        @php
                                            ...
                                            <td class="px-6 py-4">
                                                ...
                                                @elseif ($isInProgress)
                                                    <a href="{{ $consultation->meeting_link }}"
                                                        class="flex items-center justify-center px-2 py-1 font-medium text-white rounded-sm bg-primary gap-x-1">
                                                        <ion-icon class="text-white" name="play-circle"></ion-icon> Start
                                                    </a>
                                                @elseif ($isPast)
                                                    <form
                                                        action="{{ route('completedAppointments', ['appId' => $consultation->appointment_id]) }}"
                                                        method="POST">
                                                        @csrf
                                                        @method('PUT')
                                                        <button type="submit"
                                                            class="flex items-center justify-center px-2 py-1 font-medium text-white rounded-sm bg-primary gap-x-1">
                                                            <ion-icon class="text-white" name="checkmark-circle"></ion-icon>
                                                            Done
                                                        </button>
                                                    </form>
                                                @endif
