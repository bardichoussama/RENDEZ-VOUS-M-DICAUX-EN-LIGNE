...
<div class="flex items-center w-full h-full gap-x-4">
    <div class="w-1/2 h-full p-6 bg-white rounded-sm shadow dark:border dark:bg-gray-800 dark:border-gray-700">
        ...
        <div class="px-24 py-4 mt-12 space-y-4 ">
            <div class="mb-11">
                ...
                <p class="text-xs text-gray-400">Already using Wecare? <span class="text-primary"><a href="{{route('patientLoginView')}}">Login</a></span></p>
            </div>
            <form class="space-y-1 md:space-y-6" method="post" action="{{ route('patientSignupView') }}">
                @csrf
    ...
    <div class="w-1/2 h-full bg-black rounded-sm ">
        <img class="object-cover w-full h-full shadow-lg" src="{{ asset('assets/img/about-contact.jpg') }}"
            alt="" />
    </div>
</div>
@endsection